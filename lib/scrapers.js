...
async function fetchCheapSharkStoreDeals(storeId, platformName) {
    try {
        const { data } = await axios.get(`https://www.cheapshark.com/api/1.0/deals?storeID=${storeId}&pageSize=30&sortBy=Savings`, {
            timeout: 15000,
            headers: DEFAULT_HEADERS
        });
        if (!Array.isArray(data)) return [];
        ...
        const mapped = data
            .filter(d => Number(d.savings || 0) >= 60)
            .map(d => ({
                title: d.title,
                platform: platformName,
                originalPrice: d.normalPrice ? `$${d.normalPrice}` : 'N/A',
                currentPrice: d.salePrice ? `$${d.salePrice}` : 'N/A',
                discountPct: `-${Math.round(Number(d.savings || 0))}%`,
                link: d.dealID ? `https://www.cheapshark.com/redirect?dealID=${d.dealID}` : null,
                category: 'Discount',
        ...
        console.error(`CheapShark ${platformName} fallback error:`, e.message);
        return [];
    }
}
...
async function fetchSteamFreeDeals() {
    ...
    // If there are no active free-to-keep Steam offers, surface deep store discounts.
    try {
        const { data } = await axios.get(searchUrl, {
            headers: DEFAULT_HEADERS,
            timeout: 10000
        ...
        $('#search_resultsRows a').each((i, el) => {
            const row = $(el);
            const discountPctText = row.find('.discount_pct').text().trim();
            const pct = parseDiscountPercent(discountPctText);
            ...
            const originalPrice = cleanPriceText(row.find('.discount_original_price').text().trim()) || 'N/A';
            const currentPrice = cleanPriceText(row.find('.discount_final_price').text().trim()) || 'N/A';
            if (!currentPrice || currentPrice === 'N/A') return;
        ...
        discounted.sort((a, b) => parseDiscountPercent(a.discountPct) - parseDiscountPercent(b.discountPct));
        return finalizeDeals(discounted.slice(0, 20));
    } catch (e) {
        console.error('Steam discount fallback error:', e.message);
        return finalizeDeals(deals);
    }
}
...
async function fetchGOGDeals() {
    ...
    try {
        ...
        const freeDeals = finalizeDeals(deals);
        if (freeDeals.length > 0) {
            return freeDeals;
        }

        // If no free GOG giveaway is active, include deep GOG discounts instead.
        const discounted = [];
        $('a.product-tile, product-tile a, a[href*="/game/"]').each((i, el) => {
            const tileLink = $(el);
            const tile = tileLink.closest('product-tile').length ? tileLink.closest('product-tile') : tileLink;
            ...
            const discountText = tile.find('.product-tile__discount, .price-discount').text().trim();
            const pct = parseDiscountPercent(discountText);
            if (pct === null || pct > -70) return;

            const finalPrice = tile.find('.product-tile__price-final, .price-final, .product-tile__price-discounted').text().trim();
            const originalPrice = tile.find('.product-tile__price-original, .price-old').first().text().trim() || 'N/A';
...
async function getAllDeals() {
    ...
    // Final safety net for empty Steam/GOG sections: use authorized store-deal feed.
    if (!hasPlatform(mergedDeals, 'steam')) {
        const steamStoreDeals = await fetchCheapSharkStoreDeals(1, 'Steam');
        mergedDeals = finalizeDeals([...mergedDeals, ...steamStoreDeals]);
    }
    if (!hasPlatform(mergedDeals, 'gog')) {
        const gogStoreDeals = await fetchCheapSharkStoreDeals(7, 'GOG');
        mergedDeals = finalizeDeals([...mergedDeals, ...gogStoreDeals]);
    }
